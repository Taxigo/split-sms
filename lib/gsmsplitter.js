...
module.exports.split = function (message, options) {
  ...
  if (fitsOneMessage()) {
    ...
    return {
      ...
      totalLength: totalLength,
      totalBytes: totalBytes,
      totalInPart: 160 - (messages.length === 1 ? singleUdhLength() : multiUdhLength())
    };
  }
      ...
      for (var i = 0; i < str.length; i++) {
        ...
        if (gsmChar === undefined) {
          ...
          if (extendedChar) {
            ...
            console.log(
                'COULD NOT FIND CHAR MAPPED "' + str[i] + '", ' +
                'CH:' + str[i] + ', ' +
                'CP:' + str[i].codePointAt(0));
          }
        }
  ...
  function isLastByte(currentCharByteLength) {
    return 160 - multiUdhLength() - currentCharByteLength - 1;
  }
